...
////////////////////////////////////////////////
//Variables
#define MAZE_DIMENSIONS_X   16
#define MAZE_DIMENSIONS_Y   16

#define MAZE_TARGET_X   8
#define MAZE_TARGET_Y   8

#define MAZE_HOME_X     0
#define MAZE_HOME_Y     0

//predefined in arduino
...
//FloodFill Target
uint8_t                 by_MAZE_TARGET_X;
uint8_t                 by_MAZE_TARGET_Y;
...
WallState       Wall_H[MAZE_DIMENSIONS_X+1][MAZE_DIMENSIONS_Y];
//Maze Wall - Vertical
WallState       Wall_V[MAZE_DIMENSIONS_X][MAZE_DIMENSIONS_Y+1];
//Maze Cells
MyMaze_Cells    Maze_Cells[MAZE_DIMENSIONS_X][MAZE_DIMENSIONS_Y];
//Rat Coordinates
...
//State of Maze Exploration
typedef enum 
{
MAZE_CALIBRATE,                   //Autozero the LiDARs and the Gyro
MAZE_EXPLORE,                     //Map the Maze and find a solution
MAZE_RETURN_HOME,                 //Return Home with a solution
MAZE_FEED_THE_RAT_SOME_CHEESE,    //Return Home with a solution
}Maze_State_E; 

//Maze Exploration/Solving Mode
...
////////////////////////////////////////////////
//Log functions
//todo - add mms specific / arduino specific implementations
...
void Maze_Reset()
{
    for (int i = 0; i < MAZE_DIMENSIONS_X; i++) 
    {
        for (int j = 0; j < MAZE_DIMENSIONS_Y; j++) 
        {
...
{
    for (int i = 0; i < (MAZE_DIMENSIONS_X+1); i++) 
    {
        for (int j = 0; j < MAZE_DIMENSIONS_Y; j++) 
        {
...
void initializeMaze_V() 
{
    for (int i = 0; i < MAZE_DIMENSIONS_X; i++) 
    {
        for (int j = 0; j < (MAZE_DIMENSIONS_Y+1); j++) 
        {
...
void Flood_Fill_Value(int x, int y, int loopSize)
{
    ...
    //if(Maze_Cells[x][y].UpdateFlag == false)
    {
        ...
        //Manage Maze borders
        x_plus  = constrain((x+1) , 0, (MAZE_DIMENSIONS_X-1));
        y_plus  = constrain((y+1) , 0, (MAZE_DIMENSIONS_Y-1)); 
        x_minus = constrain((x-1) , 0, (MAZE_DIMENSIONS_X-1));
        y_minus = constrain((y-1) , 0, (MAZE_DIMENSIONS_Y-1));   
...
void Maze_FloodFill(int x, int y)
{
    ...
    //Sanity check
    if( (x < MAZE_DIMENSIONS_X) && (y < MAZE_DIMENSIONS_Y) )
    {
        //Set Target Square
        ...
        loopLimit = MAX4(x, y, (MAZE_DIMENSIONS_X - x - 1), (MAZE_DIMENSIONS_Y - y - 1));
        ...
        for(int i = 1 ; i <= loopLimit ; i++)
        {
            ...
            //North
            //Prime the loopers
            x_Local = constrain((x-i) , 0, (MAZE_DIMENSIONS_X-1));
            y_Local = constrain((y-i) , 0, (MAZE_DIMENSIONS_Y-1));            
            //Loop
            for(int j = 0 ; j < loopSize ; j++)
            {
                //Move to next cell
                y_Local = y_Local + 1;
                x_Local = constrain(x_Local , 0, (MAZE_DIMENSIONS_X-1));
                y_Local = constrain(y_Local , 0, (MAZE_DIMENSIONS_X-1));
...
void Maze_Init()
{
    ...
    by_MAZE_TARGET_X = MAZE_TARGET_X;
    by_MAZE_TARGET_Y = MAZE_TARGET_Y;
    Maze_FloodFill(by_MAZE_TARGET_X, by_MAZE_TARGET_Y);
}
...
int main(int argc, char* argv[]) 
{
    ...
    if(Maze_State == MAZE_EXPLORE)
    {
        ...
        if((myRat.x == by_MAZE_TARGET_X) && (myRat.y == by_MAZE_TARGET_Y))
        {
            //Parse the maze and mark all unknown walls as present
            normalize_Maze();    

            //Go Home
            by_MAZE_TARGET_X = MAZE_HOME_X;
            by_MAZE_TARGET_Y = MAZE_HOME_Y;

            //Flood fill maze
            //update maze values
            Maze_Reset();
            //for(int i = 0 ; i < MAX(MAZE_DIMENSIONS_X, MAZE_DIMENSIONS_Y); i++)
...
void Maze_Solve()
{
    ...
    while (!((myRat.x == by_MAZE_TARGET_X) && (myRat.y == by_MAZE_TARGET_Y))) 
    {
        if(Maze_State == MAZE_EXPLORE)
        {
